...
- **Interactive TUI shell** — Vim-inspired modal keyboard control, responsive layout down to an 80×24 terminal, Team/Notification/Calendar panels.
...
- **Log viewer** — `Ctrl+4` opens a scrollback overlay of the app's own logs, colored by level, secrets automatically redacted.
...
- **Zero Setup** — no C compiler, no database server, no config file to hand-write; everything needed beyond `rustup` is either pure Rust or already on the OS.
