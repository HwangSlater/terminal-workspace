...
- **Sidebar Auto-Collapse**: If terminal width is $< 120$ characters, the body area shows only one panel at a time instead of Team/Notification/Calendar side by side. Which one follows `focused_dock` — `Tab`/`Shift+Tab` (cycle) or `Ctrl+1`/`Ctrl+2`/`Ctrl+3` (Team/Notification/Calendar directly) switch it, exactly the same shortcuts used to move focus on a wide terminal. `Ctrl+4` (Bottom/Log) has no panel of its own in this swappable slot — Log already has its own always-visible row below the body — so it falls back to showing Notification. Corrected from an earlier draft of this doc that named `Ctrl+1`/`Ctrl+4` as the Team/Calendar toggles specifically; the real binding scheme is `Ctrl+1`=Team, `Ctrl+2`=Notification, `Ctrl+3`=Calendar, `Ctrl+4`=Bottom dock (`docs/02-architecture/keyboard.md`), and prior to this fix Team/Calendar were simply unreachable below 120 columns regardless of which key was pressed.
