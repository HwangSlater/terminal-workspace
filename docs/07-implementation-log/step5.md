...
- One scope note not called out earlier: the Command Bar's `Enter` key pushes to history but does **not** yet parse/dispatch the typed text as a `Command` — there's no text-to-`Command` parser, and with only 4 `Command` variants and no live integrations, building one now would have nothing meaningful to parse into. Left as a clearly-commented gap rather than a silent one. **Closed in Phase 9** (`step9.md`) once there was something real to parse into (`SendSlackMessage`, `SetPresence`).
