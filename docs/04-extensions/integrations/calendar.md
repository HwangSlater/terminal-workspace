...
- `step33.md`: no direct unit test (`poll_one`/`poll_once` are live-network code, same "not directly unit tested" boundary as the rest of this section) — manual acceptance check: launch with an existing calendar connection and upcoming occurrences already inside the lookahead window, confirm no notification fires on that first poll, confirm a later poll's genuinely new occurrence still notifies normally.
