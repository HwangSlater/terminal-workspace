...
- **First-poll behavior, corrected (`step33.md`)**: the very first poll after every launch (not just a fresh connection) still records every currently-open PR into the seen-set, but does *not* publish a notification for any of them — the seen-set starting empty on every restart previously meant every open PR looked "new" and fired a desktop notification on every single launch, a real bug, not the deliberate one-time catch-up this doc previously described it as. A PR that shows up on any *later* poll still notifies normally.
