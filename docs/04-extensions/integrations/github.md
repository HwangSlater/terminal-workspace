...
Polling loop (`tokio::time::interval`, period = `[integrations.github].sync_interval_secs`, default 60s). `/sync` (`step46.md`) forces one cycle immediately without waiting for the timer — the same `poll_once` this loop calls, invoked once out-of-band and serialized against the loop's own tick via a shared lock so the two can't race over `seen_prs`, and always with `is_first_poll: false` so anything a manual sync turns up toasts normally rather than being marked already-read.

- **Pull requests**: `GET /repos/{owner}/{repo}/pulls?state=open` for each repository in `[integrations.github].repositories`. A PR number not previously seen (tracked in an in-memory `(repo, pr_number)` set, per adapter instance) maps to `NotificationItem`: `source = IntegrationSource::GitHub`, `title` = `"{repo}#{number} {pr title}"`, `body` = `"by {author login}"`, `timestamp_ms` from the PR's `updated_at` (ISO 8601), `priority = PriorityLevel::Medium`, `action_link` = the PR's `html_url`.
- **First-poll behavior, corrected (`step33.md`, `step39.md`)**: the seen-set starts empty on every restart, so without special handling every currently-open PR would look "new" and fire a desktop notification on every single launch — a real bug, not the deliberate one-time catch-up this doc previously described it as. The very first poll after every launch still records every currently-open PR into the seen-set and **still publishes normally** (`step39.md` — `step33.md`'s original fix skipped the publish entirely here, which left the Notification panel unable to show a PR opened moments before/during a restart, since the seen-set already marked it recorded either way and no later poll would ever see it as new again); the item's `is_read` is set to `true` for this first-poll case instead, so only the desktop toast is suppressed. A PR that shows up on any *later* poll publishes with `is_read: false` and notifies normally.
...
`repositories` no longer needs hand-editing `config.toml` — the `Ctrl+R` overlay fetches the authenticated user's accessible repositories via `GET /user/repos` (page-paginated, stops once a page returns fewer than 100 results). Selecting rows and pressing `Enter` dispatches `Command::ApplySelection{source: IntegrationSource::GitHub, items}` (generalized in `step11.md` from `Command::ApplyGitHubSelection`, once Calendar-shaped single-list selections proved the pattern repeats), which overwrites `config.toml`'s `repositories` (see Configuration below) and restarts the poll loop with them immediately — no restart required. Manually editing `config.toml` still works too; the picker is a convenience, not the only way in. **`step37.md`**: `/` starts a live, case-insensitive filter against the fetched `owner/repo` labels (client-side, mirrors the Slack picker's), `Enter` stops typing and returns to browsing the narrowed list. **`step41.md`**: `/repo-watch owner/repo [owner/repo2 ...]` dispatches the same `Command::ApplySelection` from the command line, replacing the full repository list in one shot. **`step45.md`**: `Tab` autocompletes every `owner/repo` argument of `/repo-watch` (an open-ended list, unlike `/send`'s single channel argument), matched case-insensitively by prefix against whatever `Ctrl+R` last fetched.
