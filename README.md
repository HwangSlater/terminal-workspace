...
4. `Ctrl+K`로 연결된 캘린더 목록을 보고, 제거하고 싶은 걸 체크 해제한 뒤 저장하면 됩니다. 커서를 올린 상태에서 `e`를 누르면 이름만 바로 바꿀 수 있습니다 (주소를 바꾸려면 제거 후 다시 추가하세요). 명령줄에서 `/calendar-rename <기존 이름> <새 이름>` / `/calendar-remove <이름>`으로도 가능합니다.
