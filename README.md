...
| `Enter` | 선택한 알림/일정을 읽음 처리 (명령줄에서 `/read-all`로 안 읽은 알림 전체 처리) |
| `:` | 명령줄 입력 모드로 전환 |
...
5. 명령줄(`:`)에서 바로 사용할 수 있습니다: `/send #채널이름 메시지`, `/away`·`/active`·`/offline`·`/meeting`·`/lunch [메시지]`로 상태 변경, `/slack-watch #채널1 #채널2 ...`로 메시지 알림 받을 채널 목록을 통째로 교체(`Ctrl+P` 없이). 명령어/채널명 입력 중 `Tab`으로 자동완성됩니다.
...
3. `Ctrl+R`로 접근 가능한 저장소를 선택하면, 열린 PR이 알림 패널에 표시됩니다. 마찬가지로 `/`로 검색할 수 있습니다. 명령줄에서 `/repo-watch owner/repo owner/repo2 ...`로도 저장소 목록을 교체할 수 있습니다.
...
4. `Ctrl+K`로 연결된 캘린더 목록을 보고, 제거하고 싶은 걸 체크 해제한 뒤 저장하면 됩니다. 커서를 올린 상태에서 `e`를 누르면 이름만 바로 바꿀 수 있습니다 (주소를 바꾸려면 제거 후 다시 추가하세요). 목록이 길면 `/`를 눌러 이름으로 검색할 수 있습니다. 명령줄에서 `/calendar-rename <기존 이름> <새 이름>` / `/calendar-remove <이름>`으로도 가능하고, 이름 입력 중 `Tab`으로 자동완성됩니다.
5. `/calendar-range <시간>`으로 알림 패널에 몇 시간 앞의 일정까지 보일지 바꿀 수 있습니다 (기본 24시간). 캘린더 패널 폭은 일정 내용에 맞춰 자동으로 넓어지고, 아무리 길어도 줄바꿈 없이 한 줄로만 표시됩니다 (너무 길면 "…"로 줄임).
6. `Ctrl+M`을 누르면 월 단위 달력 그리드가 뜹니다 — 알림 패널의 "다가오는 일정" 목록과 별개로, 그 달 전체를 한눈에 봅니다. `←`/`→`로 날짜 이동, `[`/`]`로 이전/다음 달, 선택한 날짜의 일정은 시간과 함께 아래에 표시됩니다.
