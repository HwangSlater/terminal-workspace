...
5. `/calendar-range <시간>`으로 알림 패널에 몇 시간 앞의 일정까지 보일지 바꿀 수 있습니다 (기본 24시간).
6. `Ctrl+M`을 누르면 월 단위 달력 그리드가 뜹니다 — 알림 패널의 "다가오는 일정" 목록과 별개로, 그 달 전체를 한눈에 봅니다. 방향키로 날짜 이동, `[`/`]`로 이전/다음 달, 선택한 날짜의 일정은 시간과 함께 아래에 표시됩니다.
